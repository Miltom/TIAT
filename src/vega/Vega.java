...
import tiat.Individuum;
import tiat.IndividuumManager;
...
public class Vega {
	...
	public Vega(int anzahlBlöcke, int anzahlInds) {
		...
		for (int i = 0; i < 100; i++) {
			...
			// Zwei Blöcke mit jeweils 15 Elemente, Mutation, Pool
			
			// 15 Elemente, nach F sortiert, kuchendiagramm, mutation 1%
			Vector<Individuum> vec1 = getSelectsF1(block.get(0), anzahlInds / anzahlBlöcke);
			IndividuumManager.mutateIndsTest(vec1, 0.01);

			// 15 Elemente, nach G sortiert, kuchendiagramm, mutation 1%
			Vector<Individuum> vec2 = getSelectsF2(block.get(1), anzahlInds / anzahlBlöcke);
			IndividuumManager.mutateIndsTest(vec2, 0.01);
	...
	public void showOnDiagramm(Vector<Individuum> indsParam) {
		...
		JFreeChart chart = ChartFactory.createXYLineChart("VEGA Algorithmus", "Generation", "Fitnesswert", dataset, PlotOrientation.VERTICAL, true, true, false);
